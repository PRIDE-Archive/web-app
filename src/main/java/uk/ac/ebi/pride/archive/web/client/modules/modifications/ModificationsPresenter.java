package uk.ac.ebi.pride.archive.web.client.modules.modifications;
...
import uk.ac.ebi.pride.archive.web.client.modules.lists.ListUiHandler;
import uk.ac.ebi.pride.archive.web.client.modules.lists.ListView;
...
public class ModificationsPresenter extends Presenter<ListView<Multiset.Entry<String>>>
    ...
    public void onProteinUpdateEvent(ProteinUpdateEvent event) {
        if(!groups && event.getProteins().size() > 0) {
            ...
            Protein protein = event.getProteins().get(0);     // ARCHIVE CHANGE
            if (protein.getModifiedLocations() != null) {
                for(ModifiedLocation loc : protein.getModifiedLocations()) {
