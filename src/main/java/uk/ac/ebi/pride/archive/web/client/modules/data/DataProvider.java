...
//            String proteinID = protein.getAccession();        // ARCHIVE CHANGE
            ...
            speciesCache.put(proteinID, protein.getTaxonID());

            for(Map<String, Boolean> batchRequest : proteinRequests) {
                if(batchRequest.containsKey((proteinID))) {
                    batchRequest.put(proteinID, true);
                }
            }

            for(PeptideMatch match : protein.getPeptides()) {
//                if(peptideMatchCache.containsKey(new Pair<>(match.getSequence(), proteinID))) {   // ARCHIVE CHANGE
                if(peptideMatchCache.containsKey(new Pair<>(match.getId(), proteinID))) {
//                    peptideMatchCache.get(new Pair<>(match.getSequence(), proteinID)).add(match); // ARCHIVE CHANGE
                    peptideMatchCache.get(new Pair<>(match.getId(), proteinID)).add(match);
                } else {
                    List<PeptideMatch> matches = new ArrayList<>();
                    matches.add(match);
//                    peptideMatchCache.put(new Pair<>(match.getSequence(), proteinID), matches);  // ARCHIVE CHANGE
                    peptideMatchCache.put(new Pair<>(match.getId(), proteinID), matches);
                }
            }
